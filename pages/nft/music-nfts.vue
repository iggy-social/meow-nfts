<template>
  <Head>
    ...
    <Meta property="og:description" content="Music NFTs have come to Superposition. Check them out here!" />
  
    <Meta name="twitter:image" :content="$config.projectUrl+$config.previewMusicNfts" />
    <Meta name="twitter:description" content="Music NFTs have come to Superposition. Check them out here!" />
...
export default {
  ...
  methods: {
    async fetchNfts(clear="false") {
      ...
      // Fetch NFTs
      try {
        let url = `https://api.meownfts.com/endpoints/musicNfts?limit=${this.limit}`;
