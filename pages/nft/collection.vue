<template>
    ...
    <div class="card-body">
      ...
      <div class="row">
        ...
        <div class="col-md-5 text-center mb-3">
          ...
          <!-- Actions dropdown -->
          <div class="dropdown mt-3" v-if="showThisAppFeatures">
        ...
        <div class="col-md-7">
          <div class="mt-1 mb-4 muted-text" style="font-size: 14px;">
            ...
            <p class="me-4" v-if="showThisAppFeatures">
          ...
          <!-- Buttons -->
          <div class="row mb-3" v-if="showThisAppFeatures">
          ...
          <small v-if="isActivated && isSupportedChain && showThisAppFeatures">
            <em>
  ...
  <div v-if="!userTokenId && !isCurrentAddressOwner && showThisAppFeatures" class="card border mt-3 scroll-500">
    <div class="card-body">
  ...
  <div :key="userTokenId" v-if="userTokenId || isCurrentAddressOwner || !showThisAppFeatures">
    <!-- Media section -->
...
export default {
  ...
  data() {
    return {
      ...
      priceSellWei: null,
      readMore: false,
      showThisAppFeatures: true, // show features specific to NFTs created via this dApp
  ...
  computed: {
    ...
    removedFromFrontend() {
      // a list of NFT addresses that were launched via this dApp, but were removed from the frontend 
      // (due to various reasons, e.g. art theft, inappropriate content, etc.)
      return [
        "".toLowerCase(), // TODO: enter banned NFT address here
      ];
    },
  ...
  methods: {
    ...
    async buyNft() {
      ...
      try {
        ...
        if (receipt.status === 1) {
          ...
          try {
            await axios.get('https://api.meownfts.com/endpoints/collections/update?scope=mint&nftAddress='+this.cAddress);
            await axios.get('https://api.meownfts.com/endpoints/user-nfts/add?nftAddress='+this.cAddress+'&userAddress='+this.address);
          } catch (e) {
            console.error(e);
          }
    ...
    copyFrameLink() {
      let frameLink = "https://api.meownfts.com/frame/nft/"+this.cAddress;
    ...
    async getCollectionDetails(refresh=false) {
      ...
      if (collection?.mdAddress) {
        ...
        try {
          ...
          console.log("This NFT contract was very likely not created via this dApp.");
    ...
    async getCollectionDetailsFallback() {
      // this function is called if the NFT contract was not created via this dApp
      this.showThisAppFeatures = false;
    ...
    async sellNft() {
      ...
      try {
        ...
        if (receipt.status === 1) {
          ...
          try {
            await axios.get('https://api.meownfts.com/endpoints/collections/update?scope=burn&nftAddress='+this.cAddress);
            await axios.get('https://api.meownfts.com/endpoints/user-nfts/add?nftAddress='+this.cAddress+'&userAddress='+this.address);
          } catch (e) {
            console.error(e);
          }
