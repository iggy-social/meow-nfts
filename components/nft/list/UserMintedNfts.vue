  ...
  methods: {
    async addNftToApi() {
      ...
      try {
        ...
        let url = `https://api.meownfts.com/endpoints/user-nfts/add?nftAddress=${this.nftAddressToAdd}&userAddress=${this.uAddress}`;
        await axios.get(url);
    ...
    async fetchNfts(clear="false") {
      ...
      // Fetch NFTs
      try {
        let url = `https://api.meownfts.com/endpoints/mintedNftsByUser?addr=${this.uAddress}&limit=${this.limit}`;
